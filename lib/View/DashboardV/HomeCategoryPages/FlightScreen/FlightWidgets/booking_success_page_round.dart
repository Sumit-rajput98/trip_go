...
import 'package:trip_go/ViewM/FlightVM/flight_booking_details_round_view_model.dart';
import 'package:trip_go/constants.dart';
import 'package:intl/intl.dart';
import '../../../../../ViewM/FlightVM/flight_booking_details_view_model.dart';

class BookingSuccessPageRound extends StatefulWidget {
  final String? pnr;
  final String? traceId;
  final int? bookingId;
  final int? paymentPrice;
  final String? pnrIb;
  final int? bookingIdIb;

  const BookingSuccessPageRound({super.key,required this.paymentPrice, required this.pnr, required this.traceId, required this.bookingId, required this.pnrIb, required this.bookingIdIb});

  @override
  State<BookingSuccessPageRound> createState() => _BookingSuccessPageRoundState();
}

class _BookingSuccessPageRoundState extends State<BookingSuccessPageRound> {
  late final FlightBookingDetailsRoundViewModel viewModel;

  @override
  void initState() {
    super.initState();
    viewModel = FlightBookingDetailsRoundViewModel();
    fetchBookingDetails();
  }
  ...
  Future<void> fetchBookingDetails() async {
    await viewModel.loadBookingDetails(
      ...
      pnr: widget.pnr ?? "",
      bookingId: widget.bookingId ?? 0, pnrIb: widget.pnrIb ?? "", bookingIdIb: widget.bookingIdIb ?? 0,
    );
  ...
  Widget _inboundFlightInfoCard() {
    ...
    final bookingDetails = viewModel.bookingDetails;
    final itinerary = bookingDetails?.data.inboundItinerary;
    final segments = itinerary?.segments ?? [];

    if (segments.isEmpty) return const SizedBox(); // Handle missing data safely
              ...
              children: [
                ...
                Expanded(
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.end,
                    children: [
                      const Text("Origin", style: TextStyle(color: Colors.grey, fontFamily: 'poppins', fontSize: 12)),
                      Text(
                        "${segments[0].originAirportCode} • ${segments[0].originAirportName}",
              ...
              children: [
                ...
                Expanded(
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.end,
                    children: [
                      const Text("Destination", style: TextStyle(color: Colors.grey, fontFamily: 'poppins', fontSize: 12)),
                      Text(
                        "${segments[0].destinationAirportCode} • ${segments[0].destinationAirportName}",
                        style: const TextStyle(fontWeight: FontWeight.bold, fontFamily: 'poppins', fontSize: 12),
                        softWrap: true,
            ...
            Row(
              children: [
                Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    const Text("Duration 1h 45min", style: TextStyle(color: Colors.grey, fontFamily: 'poppins')),
                    Text("PNR: ${itinerary?.pnr}", style: const TextStyle(fontWeight: FontWeight.bold, fontFamily: 'poppins')),
                  ],
                ),
                const Spacer(),
